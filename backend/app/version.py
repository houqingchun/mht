...
| `deploy/build_package.py` | 正则读这个文件，拼成 `1.0.0+YYYYMMDD` 写进 `package-info.txt` |
...
# 界面上显示的那一串。「V1.0」＝ 主.次，丢掉修订号：`1.0.1` 仍然显示 `V1.0`，
# 那正是补丁号的意思（对外是同一个版本），而 `1.0.0+20260919` 那个构建戳负责
# 区分同一条发布线上的不同次出包。
#
