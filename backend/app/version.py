...
| `deploy/build_package.py` | 正则读这个文件，拼成 `1.1.2+YYYYMMDD` 写进 `package-info.txt` |
...
# 界面上显示的那一串。「V1.1」＝ 主.次，丢掉修订号：`1.1.2` 显示 `V1.1`，
# 那正是补丁号的意思（对外是同一条发布线），而 `1.1.2+20260920` 那个构建戳负责
# 区分同一条发布线上的不同次出包。
#
