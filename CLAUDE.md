...
### 32. V1.1.5：两处「效度建议复测」（2026-09-24）

用户报了两件事，它们在界面上是**同一个名字**（「效度建议复测」），而根因毫无关系：

> 1. 统计分析中 全校八维度分析 》 效度建议复测 数字是对的，但全校预警总览 > 效度复测建议 是错误的
> 2. 统计分析中 全校八维度分析 》 效度建议复测 需要能够导出学生列表信息，以便知道哪些学生需要
>    重新测试（学生基本信息要完整）

第一件是一个**恒空的分组**，第二件是**一个新导出**。两件都落在 `analytics` 这一块。

#### 问题 1：一个「看起来有数据、其实是另一个表」的读者

`DimensionsPage.vue`（全校八维度分析）与 `OverviewPage.vue`（全校预警总览）此前读的是
**两个不同的东西**，而它们的标题逐字相同：

| 页面 | 此前读什么 | 结果 |
|---|---|---|
| 全校八维度分析 | `sample_quality.validity_flagged_count` | 对 |
| 全校预警总览 | `signal_type_stats` 里 `RETEST_RECOMMENDED` 那一组 | **恒 0** |

那一组**恒空**，它不是一个「算错了」，是**两张表之间的一次张冠李戴**：
`RETEST_RECOMMENDED` 是 `assessment_result.validity_status` 的取值，而
`risk_event.signal_type` 是**另一套码**（§21 的 `SIGNAL_TYPE_BY_RISK_TYPE`：六档
`risk_type` 映到三档 `signal_type`）。所以用后者去查前者，那个键在任何一行上都不存在，
`signalCount()` 每次都命中它的 `|| 0` 兜底——**而屏幕上写着一个 0，看起来像「本次普查
没有人需要复测」**。

这与 §19 那条「`/openapi.json` 报的版本号没人设过、却看起来像设过」是同一个形状的另一面：
那一次是「一个没人设过的值恰好等于真值」，这一次是「一个查询查错了表，于是它的结果
永远是兜底值」。**两者共同的性质是：屏幕上一切正常，而没有任何东西会因此变红**——
后端发的是完全正确的 `signal_type_stats`，前端也完全正确地渲染了它。

修法是把那两处**接到同一个字段上**（`sample_quality.validity_flagged_count`），并把这个
由来写进 `OverviewPage.vue` 的那一段注释里（那段注释现在就立在那两行上面，写着
「**不读 `signal_type_stats`**」以及为什么）。

**判据是一条 e2e**（`both report pages agree on the validity retest figure`），它按
**两个页面互相比**来断，而不是各自断一个数：这一条要证明的正是「两处口径一致」，
写死一个期望值只能证明「其中一处此刻等于 4」。变异验证 2/2：
M1 把 `OverviewPage.vue` 改回 `signalCount('RETEST_RECOMMENDED')` → `Expected: 2 / Received: 0`；
M2 让取数的 `numberIn` 恒返回 0 → 红在 `toBeGreaterThan(0)`（**先证明有东西可数**那条，
没有它 M2 在一个「两边都是 0」的库上会绿）。

#### 问题 2：`POST /analytics/validity-retest/export`

在那之前，「效度建议复测 3 人」这句话只有**人数**——「这 3 人是谁」在界面上没有任何出路。
现在八维度分析那一格下面多一枚按钮，导出的是**一份实名派工单**。

**三道门槛**，逐字照 `api/v1/analytics.py` 那段 docstring：
...
第三道是这个端点唯一有内容的一道，而它**必须**在服务层：前两道都是
「能看报表」「能做受控导出」，而这份文件逐行印着学号、姓名、班级。德育领导两条都过得了
（`AGGREGATE_STATS` 是 `SCHOOL`、`CONTROLLED_EXPORT` 是 `PROGRESS_SUMMARY`），真正拦住
他的是最后那一道——**少了它，§4 那句「受控导出不能成为绕过心理详情的旁路」在这个端点上
就是一句空话**。三条门槛用例各断一次（领导 403 `ROLE_FORBIDDEN`、管理员 403、
`purpose` 必填 422 `PURPOSE_REQUIRED`），其中领导那一条**先证明他够得着这一页的报表**
（`GET /analytics/report` 200）——否则下面的 403 可能来自别处。

**名单与那个数同源，而且是构造上的同源**（§11）。`validity_retest_students` 与
`analytics_report` 的 `sample_quality` 共用两样东西：同一批场次（都来自 `_report_scope`
的 `calculated`，不在这里另写一遍「每名学生取最近一场」的推导）与同一个判据
（都调 `_is_validity_flagged`）。主用例三处一起断——报表的
`validity_flagged_count`、作业行的 `row_count`、文件里的数据行数——**只断前两个的话，
一个「作业记 1 行、文件里 0 行」的实现照样是绿的，而拿到文件的人看到的正是文件**。

三处容易改错、各自都有理由的：

- **不套 `analysis_mode`。** 名单读的是 `calculated`（每名学生最近一场已计算），不是
  `included`（按所选统计模式纳入聚合的那些）——效度提示与统计模式无关。所以**导出请求里
  没有这个字段**，而这不是「不支持」：报表在两个模式下给出同一个 `validity_flagged_count`，
  带一个不生效的字段只会在请求体里多一处看不懂的东西。
- **一人一行，按 `student_id` 去重，取效度分最高的那一场。** 同一个人在两场里都触发时，
  「他要重测」这件事只该在名单上出现一次（这是一份用来点人的派工单，重复行会让人以为有
  两个学生）。**按 id 不按姓名**：同名的两名学生是两个人。
- **学生信息取现名册，不取目标行快照**，且年级 / 班级是**外连接**。这一条与 §22 那条
  快照读法**方向相反**，而两条都对：快照回答的是「发放那一刻学校看到的是谁」（完成率
  报表要的），这份文件回答的是「**今天**去哪找这名学生」。外连接照 `dist/check_validity_score.sql`
  的先例：名册上班级缺失的学生仍然要出现在名单里，内连接会把他**静默丢掉**，
  而名单少一个人的代价比某一格空着大得多。

**列（`VALIDITY_RETEST_COLUMNS`，11 列）**：学号 / 姓名 / 年级 / 班级 / 性别 / 年龄 /
学籍状态 / 测评任务 / 测评日期 / 来源 / 效度分。用户那句「学生基本信息要完整」在用例里
是**逐列断的**（姓名、年级、性别、以及「测评任务那一列不该是空的——否则这份名单答不上
是哪一场测出来的」）。数值列（年龄 / 效度分）没有值时**留空**、不留 `—`（§3 那条：
`—` 会让表格软件把整列当成文本）。`row_count = len(items)`，**不数表头那一行**（§29）。

`mask_level` 记 `IDENTIFIED`：这份文件是派工单，遮蔽了就没法派人，所以实名是**必须的**
而不是宽松；遮蔽模式同时进 `export_job.mask_level`（机器判据）与审计的 `detail`
（能搜到的那一份轨迹），两条一起断（§8）。`field_policy` 照 §29 从**产物**倒着写
（`list(document.columns)`），请求体里没有列名的落点。

**8 条后端用例**（`test_validity_retest_export.py`，新文件）里有一条值单独说：
`add_unflagged_classmate` **是判据本身，不是陪衬**。基线种子名册上只有一名学生（S001），
而他正好就是被标记的那一个——**此时「导出被标记的人」与「导出全部人」给出同一个结果**，
摘掉 `_is_validity_flagged` 那道过滤，主用例每一条断言照样是绿的。所以那个同班的
效度正常同学必须真的存在、真的落进 `calculated`（用例先断 `completed_count == 2`
再断 `kpi == 1`）。这是 CLAUDE.md 测试注意里那条的又一次发作：**断言「这里没有他」，
得先真的有一个他**。他走 `score_session` 而不是手插一行 `AssessmentResult`，
理由写在那个函数的 docstring 里（手插的行缺八维度结果，于是「把一个人插进去」这件事
本身会变成另一个变量）。

#### ★ 契约第一面在这个码上**没有自动守卫**，那是一条人工同步点

新增 `EXPORT_TYPE_VALIDITY_RETEST` 之后要补**两处**镜像：`test_status_vocabulary.py` 的
`EXPORT_TYPES` 集合与 `labels.ts` 的 `EXPORT_TYPE_LABELS`（现为七项）。补之前那一跑是
**`1 failed, 777 passed`**，红的就是那条。

**但那条守卫并不像它的断言文案说的那样工作，这一条值得记**：
`test_export_job_vocabulary_is_the_documented_one` 比的是「`export_service` 的模块常量 ↔
**本测试文件里手工维护的集合**」——它**不读 `labels.ts`**。所以：

| 变异 | 结果 |
|---|---|
| 删掉 `labels.ts` 的 `VALIDITY_RETEST: '效度复测名单'` | **不红**（`test_status_vocabulary.py` 与 `test_export_labels_match_frontend.py` 都绿） |
| 摘掉 `test_status_vocabulary.py` 集合里的 `"VALIDITY_RETEST",` | **红**，报 `服务端多出 {'VALIDITY_RETEST'}，前端多出 set()` |

也就是说它是一处**人工同步点**（新增后端码时逼人去补前端），而**不是**自动比对。
`labels.ts` 那四十多张表里只有 **9 张**（`export_labels.py` 镜像的那些——「会进导出文件的
才镜像」，§29 写着理由）有自动 ① 面守卫，`EXPORT_TYPE_LABELS` 不在其中。这是**既有架构
状态，不是本期引入的缺口**。

**第一发变异打歪了，教训在这里**：我删 `labels.ts` 那一条、见它不红，一度据此刻判定
「`EXPORT_TYPE_LABELS` 没有任何一面在守」。这与 §28 的 M2 是同一条——**变异没红时，
先怀疑变异写错了，不是守卫失灵**。而它还有一层更细的：那条用例的**断言文案**写着
「与 labels.ts 里那张表对不上」，**超出了它实际的判据范围**，于是它把做变异验证的人
引到了靶子的另一边。这是「一条恒绿的守卫比没有更糟」的镜像另一面：
**一处判据的措辞如果超出它的判据范围，会让下一个验证它的人打错靶**。

#### 两条刻意不做（写在这里，免得下次被读成漏了）

1. **问题 2 的前端不写 e2e 用例。** 新增一条会写 `resource_type=EXPORT` 审计的 e2e，
   在 `playwright.config.ts` 的 `fullyParallel` 下会与审计页那个只显示最新 20 行的用例
   **抢同一条时间线**——与 §24 里 `STUDENT` 被移出 `UNTRANSLATED_CODES` 的成因逐字同源
   （「一条会无故变红的守卫很快会被人关掉」）。这一条链的服务端侧已被 8 条后端用例 +
   下面那次真实回路钉住，而「视图有没有把那枚按钮渲染出来」是**这一条取舍让掉的**。
2. **新码 `VALIDITY_RETEST` 不进 `UNTRANSLATED_CODES`。** 同一张表另外六个导出类型码
   **都不在**清单里；只加第七个会造成「同一张表七个码只有一个被守」的不一致，而且它
   成立的前提是「开发库里恰好留着一行 `VALIDITY_RETEST` 作业」——**一个数据依赖的脆弱
   判据**。要收它就连另外六个一起收，那是另一件事。

#### 跑数与守卫

`make test` **778 passed / 0 failed / 471.40s**（+8 条，即上面那个新文件）；
`make e2e` **136 passed (30.9s)**（+1 条，即问题 1 那条两页比对）。
两次全量之间做了变异验证：问题 1 那组 **2/2**、问题 2 那组 **4/4**（其间一次打歪，
见上），收尾又补了两次定向变异，每一次都 `cp -p` 落盘备份 + `cmp` **逐字节**还原。

**真实服务 + 真实开发库的回路走过一次**（问题 2 那一条链唯一能被端到端看见的地方）：
报表 `validity_flagged_count = 2`、作业 `EXPORT-20260924-6` 记 2 行、下载回来的 CSV 是
**2 行数据**（`S041 | 卫睿泽 | 初三 | 1班 | 女 | 15 | 在读 | … | 8`），而同一场的
`completed_count = 49`——**这个对比就是「名单确有筛选」那句话的可执行形式**；
`mask_level = IDENTIFIED`，审计两条（`导出效度复测名单` / `下载导出文件`）。

**残留（如实记，不清理）**：那次回路在共享开发库留下了一行 `export_job`（无删除接口、
