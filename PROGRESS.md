...
**剩下三件事都不是本轮编码动作**：
1. §5.9 另外**五条**待你裁决（第 5 条「新版本重算快照」已裁决并落地）；
2. **要不要把 `__version__` 升到 `2.0.0`**——`version.py` 今天仍是 `1.1.6`，
   见 §5.7 末尾那一段（升版本在这条线上一直是**独立提交**，所以没有塞进这次）；
3. 出 2.0.0 的部署包（`make deploy-package`）——要不要出、什么时候出，等你发话。
